...
async def start_command(message: types.Message, state: FSMContext):
    ...
    if message.chat.type == 'private':  # Обробка приватного чату
        ...
        greeting_text = f'Привіт, <b>{first_name}!</b> 😊\n💼 Я бот для конвертації тексту у голос і навпаки 🎙️\n\n<b>Щоб використовувати мої функції у групі:</b>\n✅ Надішліть /voice <i>текст</i> для конвертації\n✅ Використовуйте /lang для вибору мови\n✅ Надішліть /text у відповідь на голосове повідомлення яке потрібно конвертувати у текст'
...
async def voice_to_text(message: types.Message):
        if message.chat.type in ['group', 'supergroup']:
            if not message.reply_to_message or not message.reply_to_message.voice:
                await message.reply("🎙 Будь ласка, використовуйте\n/text у відповіді на голосове повідомлення!")
                return
    ...
    # Завантажуємо голосове повідомлення
            async with aiohttp.ClientSession() as session:
                async with session.get(f"https://api.telegram.org/file/bot{TOKEN}/{file.file_path}") as resp:
                    with open(file_path, "wb") as f:
                        f.write(await resp.read())

    # Конвертуємо .ogg у .wav для SpeechRecognition
            ...
            try:
                text = recognizer.recognize_google(audio_data, language="uk")
...
async def process_message(message: types.Message, state: FSMContext):
    if message.chat.type == 'private':
        ...
        if message.text == '🗣️Вибрати голос':
        ...
        elif message.text == 'Голос ➡ текст':
            file_path = f"voice_{message.from_user.id}.ogg"
            wav_path = file_path.replace(".ogg", ".wav")

            if not os.path.exists(wav_path):
                await message.reply("Немає збереженого голосового повідомлення. Надішліть нове!")
                return
            ...
            convup = ReplyKeyboardMarkup(resize_keyboard=True)
            conv1 = KeyboardButton('Текст ➡ голос')
            conv2 = KeyboardButton('Голос ➡ текст')
            ...
            await state.finish()
            await message.answer("<b>Оберіть спосіб\nконвертації нижче</b>", reply_markup=convup, parse_mode=types.ParseMode.HTML)
...
async def convert_to_voice(message: types.Message, state: FSMContext):
    ...
    if text in ['🗣️Вибрати голос', 'Інструкція📜', '🔙Меню', '⭐️Збережене', 'Текст ➡ голос', 'Голос ➡ текст', 'Конвертація🔊']:
        await state.finish()
...
async def handle_voice(message: types.Message):
    if message.chat.type == 'private':
        ...
        # Завантажуємо голосове повідомлення через aiohttp
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.telegram.org/file/bot{TOKEN}/{file.file_path}") as resp:
                with open(file_path, "wb") as f:
                    f.write(await resp.read())
        ...
        await message.answer("Натисніть кнопку якщо хочете розпізнати текст", reply_markup=convup)
        #else:
            #await message.answer(
                #"<b>Для того щоб отримати доступ до функцій бота, вам потрібно підписатися на канал</b> 👇🏻",
                #parse_mode=types.ParseMode.HTML,
